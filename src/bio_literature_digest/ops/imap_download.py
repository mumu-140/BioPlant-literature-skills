...
def fetch_message_ids(client: imaplib.IMAP4_SSL, keyword: str) -> list[bytes]:
    quoted = keyword.replace('"', "")
    for criteria in (f'TEXT "{quoted}"', f'SUBJECT "{quoted}"', "ALL"):
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", required=True, help="Directory where attachments will be saved.")
    parser.add_argument("--limit", type=int, default=0, help="Optional limit on matched messages. 0 means no limit.")
    return parser.parse_args(argv)


def run_imap_attachment_download(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        ids = fetch_message_ids(client, args.keyword)
        matched_messages = 0
...
def main(argv: list[str] | None = None) -> int:
    return run_imap_attachment_download(parse_args(argv))
